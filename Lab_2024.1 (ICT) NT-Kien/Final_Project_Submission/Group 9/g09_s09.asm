.data
    ...
    output_filename:    .string "/Users/truonglinhduyen/Documents/output.txt"
    out_fd:     .word   0
    buffer_number:     .space  12
    msg_file_error_open:  .string "\nError writing to output file\n"
    char_minus:      .string "-"
    # Them buffer tam cho sap xep tron
...
read_numbers:
    ...
    # Reset count
    la t1, count       # Tai dia chi cua count
    sw zero, 0(t1)     # Luu gia tri zero vao count
...
inner_loop_bubble_sort:
    ...
    # So sanh cac phan tu ke tiep nhau
    slli t2, t0, 2
    add t2, s0, t2
    lw t3, 0(t2)      # arr[j]
    lw t4, 4(t2)      # arr[j+1]
...
# Ham tro giup chuyen doi so thanh chuoi
number_to_string:
    # a0 = dia chi buffer
...
# ============================================
# Ghi ket qua vao file (da cap nhat)
# ============================================
write_results:
    addi sp, sp, -16
    sw ra, 12(sp)
